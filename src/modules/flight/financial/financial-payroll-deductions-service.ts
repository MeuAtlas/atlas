...
export async function buildFlightPayrollTaxEstimate(importId: string, ownerUserId?: string) {
  ...
  const { data: estimate, error } = await supabase.from("flight_payroll_final_estimates").select("user_id,gross_amount_minor_units").eq("import_id", importId).single();
  if (error || !estimate || (ownerUserId && estimate.user_id !== ownerUserId)) throw new Error("Fechamento bruto não encontrado.");
  const { data: rows, error: deductionError } = await supabase.from("flight_payroll_personal_deductions").select("id,name,calculation_type,amount_minor_units,percentage_basis_points,deductible_from_irrf_base,effective_from,effective_to").eq("user_id", estimate.user_id).eq("active", true);
  if (deductionError) throw new Error(deductionError.message);
  const personal: PersonalDeduction[] = (rows ?? []).filter(row => row.effective_from <= "2026-08-01" && (!row.effective_to || row.effective_to >= "2026-08-01")).map(row => ({ id: row.id, name: row.name, calculationType: row.calculation_type as PersonalDeduction["calculationType"], amountMinorUnits: row.amount_minor_units, percentageBasisPoints: row.percentage_basis_points, deductibleFromIrrfBase: row.deductible_from_irrf_base }));
