...
test("consulta é limitada ao período e aos campos da lista", () => {
  ...
  assert.match(query, /if \(isOpenCycle\)[\s\S]*forCycle\(\)[\s\S]*gte\("purchase_date", cutoffGraceFrom\)/);
  assert.match(query, /if \(isOpenCycle\)[\s\S]*eq\("bill_forecast_date", selectedCycle\.reference_month\)[\s\S]*gte\("posting_date", period\.from\)/);
  assert.doesNotMatch(query, /openProjectionCardIds/);
  assert.match(query, /getOpenCardCycleMovements\(rawMovements\)/);
  ...
  assert.match(query, /\.from\("card_installment_occurrences"\)[\s\S]*\.eq\("competence_month", selectedCycle\.reference_month\)[\s\S]*\.eq\("status", "projected"\)/);
  assert.match(query, /\.in\("card_id", cycleCardIds\)/);
...
test("interface alterna período bancário e fatura real com estado vazio responsivo", () => {
  ...
  assert.match(source, /Projeção da fatura/);
  assert.match(source, /Compras lançadas e parcelas previstas nesta fatura/);
  assert.match(source, /Parcelas previstas/);
