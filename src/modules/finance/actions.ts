...
export async function updateCardInstrumentPaymentResponsibility(data:FormData){
 const {supabase,user}=await requireFinanceAccess();
 const id=textField(data,"id",40);
 const personId=optionalText(data,"payment_responsible_person_id",40);
 const instrument=await supabase.from("credit_card_instruments").select("id").eq("id",id).eq("owner_id",user.id).maybeSingle();
 if(instrument.error||!instrument.data)throw new Error("Cartao adicional nao encontrado ou sem permissao.");
 if(personId){
  const person=await supabase.from("financial_people").select("id").eq("id",personId).eq("created_by",user.id).eq("is_active",true).is("archived_at",null).maybeSingle();
  if(person.error||!person.data)throw new Error("Pessoa responsavel nao encontrada ou inativa.");
 }
 const result=await supabase.from("credit_card_instruments").update({payment_responsible_person_id:personId,responsibility_updated_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq("id",id).eq("owner_id",user.id).select("id").maybeSingle();
 if(result.error)throwSupabaseError(result.error,"definir responsavel pelo cartao","Nao foi possivel salvar quem paga este cartao.");
 if(!result.data)throw new Error("Cartao adicional nao encontrado ou sem permissao.");
 refreshFinance();
 redirect("/financeiro/cartoes?view=manage&toast=responsibility-updated");
}
