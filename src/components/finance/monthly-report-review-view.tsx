...
export function MonthlyNextStatementSection({ view }: { view: MonthlyReportReviewViewModel }) {
  ...
  return <section className="monthly-review-section monthly-next-statement-section"><header><div><p className="eyebrow">Fatura de {monthName(view.openStatements[0].due_date.slice(0, 7))}</p><h2>Compromisso do mês seguinte</h2></div></header><div className="monthly-next-statement-list">{view.openStatements.map(statement => <article key={statement.id}><header><div><h3>{statement.card_name}</h3><small>Aberta</small></div><strong><Money value={statement.current_open_amount} /></strong></header><dl><div><dt>Sua parte estimada</dt><dd><Money value={statement.personal_share_amount} /></dd></div><div><dt>Parte de terceiros</dt><dd><Money value={statement.third_party_share_amount} /></dd></div><div><dt>Fecha em</dt><dd>{date(statement.closing_date)}</dd></div><div><dt>Vence em</dt><dd>{date(statement.due_date)}</dd></div></dl>{statement.incomeCommitmentPercentage == null ? null : <div className="monthly-income-commitment"><span>Sua renda estimada comprometida pelo cartão</span><strong>{statement.incomeCommitmentPercentage.toLocaleString("pt-BR", { maximumFractionDigits: 1 })}%</strong><small>A sua parte atual da fatura compromete aproximadamente este percentual da renda esperada. {statement.incomeEstimated ? "Percentual estimado com base na renda real mais recente." : ""}</small></div>}<p>Esta fatura ainda pode mudar até o fechamento. Ela não é uma saída de {view.header.shortMonthLabel}.</p></article>)}</div></section>;
}
