...
begin
  ...
  -- A full legacy allocation may point to the wrong statement. An explicit
  -- user link moves that single allocation instead of creating a duplicate.
  if existing_count=1
    and existing_allocation.statement_id<>target.id
    and p_allocated_amount is null
    and abs(existing_total-abs(payment.amount))<=0.01 then
