...
begin
  ...
  if tg_table_name = 'financial_entity_counterparties' then
    ...
    if new.transaction_id is not null then
      ...
      into related_workspace, related_owner
    ...
    if not found then
      raise exception 'financial entity source movement not found';
    end if;
    ...
    if related_workspace is null then
      if not exists (
        ...
        where id = new.workspace_id
          and owner_id = related_owner
          and type = 'personal'
      ) then
        raise exception 'private movement workspace mismatch';
      end if;
      related_workspace := new.workspace_id;
    end if;
  end if;
...
comment on function public.validate_financial_entity_scope() is
  'Validates entity workspace relationships and permits private movements only in the owner personal workspace.';
