...
alter table public.expense_establishment_transactions
  ...
  add column if not exists card_purchase_id uuid
    references public.card_purchases(id) on delete cascade;
...
alter table public.expense_establishment_transactions
  ...
  check (num_nonnulls(transaction_id, card_purchase_id) = 1);

create unique index if not exists expense_establishment_transactions_card_purchase_idx
  on public.expense_establishment_transactions(card_purchase_id)
...
begin
  if tg_table_name = 'expense_establishment_rules' then
    ...
    if new.transaction_id is not null then
      select workspace_id, owner_id into target_workspace, source_owner
      from public.financial_transactions where id = new.transaction_id;
    else
      select workspace_id, owner_id into target_workspace, source_owner
      from public.card_purchases where id = new.card_purchase_id;
    end if;
    if target_workspace is null then
      select id into target_workspace from public.workspaces
      where id = new.workspace_id and owner_id = source_owner;
    end if;
