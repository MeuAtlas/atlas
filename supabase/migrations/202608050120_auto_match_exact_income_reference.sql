-- A selected income reference becomes an automatic rule for the same account,
-- exact amount and bank description on future synchronizations.
...
begin
  ...
  where c.workspace_id = new.workspace_id
    ...
    and c.expected_amount = abs(new.amount)
...
drop trigger if exists financial_transactions_auto_match_income_reference on public.financial_transactions;
create trigger financial_transactions_auto_match_income_reference after insert or update of description,amount,account_id,bank_direction,status,competence_date on public.financial_transactions for each row execute function public.auto_match_exact_income_reference();
