create or replace function public.promote_flight_schedule_import_for_owner(p_import_id uuid,p_owner_user_id uuid)
...
begin
 if p_owner_user_id is null then raise exception 'Proprietário da importação ausente.' using errcode='22023'; end if;
 ...
 if not found then raise exception 'Competência operacional não encontrada.' using errcode='42501'; end if;
 ...
 insert into public.flight_schedule_audit_logs(user_id,schedule_month_id,import_id,action,performed_by,metadata) values(p_owner_user_id,target_month.id,target_import.id,case when target_import.schedule_role='PLANNED' then 'PLANNED_SCHEDULE_IMPORTED' else 'EXECUTION_SNAPSHOT_IMPORTED' end,p_owner_user_id,jsonb_build_object('event','SCHEDULE_IMPORT_RECOVERED','oldImportId',old_import_id));
